 ...
 *CSCI 2421
 * Author: Karla Vela Lopez
 * Version: Due date 11/17/22
 * Collaborated with other classmates
 *********************************************/
...
public:
    ...
    void PrintTree(ostream & out = cout) const
    {
        if(IsEmpty() )
            out << "Empty tree" << endl;
//manipulating this to get the right printing order KVL
        else {
...
private:
    ...
    **/
    ...
    //Conditions were set up to go through each left node to find the smallest number. Only focused on
    //the left subtree due to BST having the smallest numbers on the left.
     ...
     */
    //Conditions were set up to go through each right node to find the largest number. Only focused on
    //the right subtree due to BST having the smallest numbers on the right.
     ...
     */
     //In order starts from the left most bottom node/leaf. Then it goes to the subtree root then to the right node/leaf
     //Then goes to the next left subtree node and repeats the order until it gets to the main root.Then it will move
     //to the right subtrees starting with the bottom most node/leaf on the left, then subtree root, the right node/leaf
     //etc.
    ...
    //Starts on the left substrees with the most bottom left node/leaf, then right node/leaf then subtree root.
    //Then moves to the next left subtree in the same order until it reaches the BST main root. Then this same order
    //is done on the right subtrees with the most bottom left node/leaf.
    ...
    //Starts at the BST main root and moves to the left subtree node, then the next left subtree node, and this is
    //repeated until it reaches the bottom most node/leaf. Then right node/leaf, then the next subtree node.
    //Then left node, then right node, etc. This will continue until all of the left side nodes have
    //been ordered and then it will move the right subtrees starting with the bottom most and left most node/leaf. Then
    //the right leaf/node, then subtree node, etc.
    ...
    //find height of right and left trees and then compare them to see which one is larger and add 1.
